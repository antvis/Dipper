...
export const MapStyle = () => {
  ...
  const styleOption: IWidgetProps<Items> = useMemo(() => {
    if (controls) {
      return controls.find((item: IWidgetProps) => item.type === "mapStyle").options;
    }
  }, [controls]);

  // 切换地图样式
  const clickStype = (e: any) =>{setConfig('map.style',e.target.value)}
      ...
      <Menu.Item key="1">
        <Radio.Group onChange={clickStype}>
          <Space direction="vertical">
            {styleOption && styleOption.map((item: any) => {
