...
export function MeshName() {
  ...
  useEffect(() => {
    if (selectFeatures.length) {
      const findIdBySiderbartabcontent = (panel.children || []).findIndex((item) => item.type === 'siderbartabcontent')
      const findIdMeshchart = (panel.children || []).findIndex((item) => item.type === 'meshchart')
      // TODO 报错
